...
function buildConfidenceIndex(people, edges) {
    const confidenceByPerson = new Map();
    people.forEach((p) => confidenceByPerson.set(p.id, new Set()));
    edges.forEach((edge) => {
        if (!edge.c)
            return;
...
export function createSearchEngine(people, edges, officeById) {
    ...
    function scorePerson(person, query) {
        ...
        const knownAs = (person.known_as ?? []).map((value) => norm(typeof value === 'string' ? value : value?.name ?? ''));
        ...
        if (query.dy && !dy.includes(query.dy))
            return null;
        if (query.c && !confidenceByPerson.get(person.id)?.has(query.c))
            return null;
        if (query.t && !titles.some((title) => title.includes(query.t)))
            return null;
        if (query.o && !personOffices.some((office) => office.includes(query.o)))
            return null;
        ...
        if (!hits)
            return null;
        if (hits < query.terms.length)
            score -= (query.terms.length - hits) * 18;
        if (query.dy || query.c || query.t || query.o)
            score += 10;
        return { person, score, reason };
    }
    function rankSearch(query, limit = MAX_RESULTS) {
