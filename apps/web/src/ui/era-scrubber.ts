 ...
 */
import { dynastyBandStops, scrubProgress, reigningAt, type ReigningPerson } from './era-scrubber-core.ts';
import { audioEvent } from '../audio/scriptorium-audio.ts';

interface ScrubberDeps {
  people: ReigningPerson[];
  minY?: number;
  maxY?: number;
}

function bandGradient(minY: number, maxY: number): string {
...
function ordinalLabel(p: ReigningPerson): string {
  const name = p.nm ?? p.id;
  const n = Array.isArray(p.n) && p.n.length ? ` · #${p.n[0]}` : '';
  return `${name}${n}`;
}

export function initEraScrubber(deps: ScrubberDeps): void {
  ...
  const showBubble = (year: number, ghost: boolean): void => {
