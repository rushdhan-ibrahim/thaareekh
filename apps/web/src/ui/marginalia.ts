 ...
 * No DOM access; consumed by sidebar and parity-tested against the JS mirror.
 */

/** Normalize a reign field (`[s,e]` or `[[s,e],...]`) into pairs. */
export function normalizeReigns(re: unknown): Array<[number, number]> {
  ...
  for (const seg of re as unknown[]) {
...
export function tallyMarks(n: number): string {
...
export function sealHtml(grade: string | null | undefined): string {
 ...
 */
export function reignArcSvg(
  p: { yb?: number | null; yd?: number | null; re?: unknown },
  minY: number = 1117,
  maxY: number = 1968,
  w: number = 64,
  h: number = 12
  ...
  const yd = ydRaw == null ? (yb as number) : ydRaw;
  const yLife = h - 3;
  const yReign = 3;
  const parts: string[] = [];
