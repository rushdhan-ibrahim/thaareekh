import type { PersonNode, Locale } from '../types/state';

type TranslationDict = Record<string, string>;

const D: Record<Locale, TranslationDict> = {
  en: {
    ...
    life_marker_bio: 'bio',
    life_marker_est: 'est.',
    life_consistency: 'Life Consistency',
    life_consistency_hint: 'Structured life years and biography prose are checked for consistency.',
    life_consistency_ok: 'No birth/death year conflicts were detected between life fields and biography text.',
    life_birth_conflict: 'Biography mentions birth year around {bio}, but the structured life field shows {life}.',
    life_death_conflict: 'Biography mentions death year around {bio}, but the structured life field shows {life}.',
    born: 'Born',
    died: 'Died',
    ...
    inference_rule_shared_parent_plain: 'From a shared parent, this pair is modeled as siblings.',
    inference_rule_parent_of_parent_plain: 'From two parent links in sequence, this pair is modeled in a grandparent line.',
    inference_rule_parent_sibling_plain: 'From a parent link plus a sibling link, this pair is modeled in an aunt/uncle line.',
    inference_rule_children_of_siblings_plain: 'From sibling parents and their children, this pair is modeled as cousins.',
    inference_rule_shared_parent_plain_named: '{parent} is modeled as parent of both {a} and {b}, so they are modeled as siblings.',
    inference_rule_parent_of_parent_plain_named: 'So {a} is modeled in {b}\'s grandparent line.',
    inference_rule_parent_sibling_plain_named: 'So {a} is modeled as aunt/uncle-line kin of {b}.',
    inference_rule_children_of_siblings_plain_named: 'So {a} and {b} are modeled as cousins.',
    inference_no_detail: 'No handcrafted explanation is attached yet for this inferred edge.',
    inference_how_link: 'How This Link Is Inferred',
    inference_why_relation: 'Why this relation is inferred',
    inference_technical_trace: 'Technical trace',
    inference_support_level: 'Support level',
    inference_direct_claim_missing: 'This remains inferred because nearby family links are documented, but a direct statement for this exact pair is still missing.',
    inference_grade_audience_a: 'Very strong support',
    inference_grade_audience_b: 'Strong support',
    inference_grade_audience_c: 'Moderate support',
    inference_grade_audience_d: 'Weak or contested support',
    inference_grade_audience_unknown: 'Support level not graded',
    inference_narrative_grandparent_via: '{source} is linked to {target} through {via}, placing {source} in {target}\'s grandparent line.',
    inference_narrative_aunt_uncle_via: '{source} is linked to {target} through {via}, which places {source} in an aunt/uncle line relative to {target}.',
    inference_narrative_sibling_shared_parent: '{source} and {target} are linked because both are modeled as children of {parent}.',
    inference_narrative_cousin_via_parents: '{source} and {target} are linked as cousin-line kin through their parents {p1} and {p2}.',
    inference_narrative_grandparent: '{source} is likely in {target}\'s grandparent line.',
    inference_narrative_cousin: '{source} and {target} are likely cousins.',
    inference_narrative_aunt_uncle: '{source} is likely an aunt/uncle-line relative of {target}.',
    inference_narrative_sibling: '{source} and {target} are likely siblings.',
    inference_narrative_generic: '{source} and {target} are likely connected through the same family branch.',
    inference_step_start: 'We start from documented family links.',
    inference_step_still_inferred: 'This remains inferred because ',
    inference_step_parent: '{a} is documented as a parent of {b}.',
    inference_step_sibling: '{a} and {b} are documented as siblings.',
    inference_step_spouse: '{a} and {b} are documented as spouses.',
    inference_step_kin: '{a} and {b} are documented as kin.',
    inference_step_sources_support: 'Supporting family evidence is recorded in the cited sources.',
    ...
    verification_status: 'Ledger status',
    verification_panel: 'Verification Ledger',
    verification_loading: 'Loading verification metadata...',
    verification_not_indexed: 'No verification row found for this edge key in the current ledger snapshot.',
    verification_review_status: 'Review',
    verification_canonical_decision: 'Canonical',
    verification_claim_id: 'Claim',
    verification_primary_source: 'Primary source',
    verification_reviewer: 'Reviewed by',
    verification_claim_excerpt: 'Claim excerpt (ledger summary)',
    verification_citation_locator: 'Citation locator (ledger summary)',
    verification_notes: 'Ledger notes',
    verification_open_relationship_ledger: 'Open relationship ledger',
    verification_open_inference_tracker: 'Open inference tracker',
    // --- Sidebar redesign (3-tab) ---
    story: 'Story',
    offices_roles: 'Offices & Roles',
    held_offices: 'Held Offices',
    historical_context: 'Historical Context',
    offices_in_era: 'Offices in this era',
    evidence_strength: 'Evidence Strength',
    evidence_sources_total: 'Sources',
    evidence_links_total: 'links',
    crown_office: 'Crown',
    ...
    person_era_label: 'Person\u2019s era',
    no_held_offices: 'No office records for this person.',
    geographic_context: 'Geographic Context',
    // --- Office first-class entities ---
    office_summary: 'Summary',
    office_function_evolution: 'Function Across Eras',
    office_alt_names: 'Alternative Names',
    office_holders: 'Office Holders',
    holders_word: 'holders',
    no_holders: 'No holders recorded.',
    office_word: 'Office',
    office_detail: 'Office Detail',
    periods_documented: 'Documented eras',
  ...
  dv: {
    ...
    inference_logic_steps: 'How this inference is derived',
    inference_no_bases: 'No explicit basis sources are attached to this inferred edge.',
    // --- ސައިޑްބާ ރީޑިޒައިން (3 ޓެބް) ---
    story: 'ވާހަކަ',
    offices_roles: 'މަޤާމާއި ދައުރު',
    held_offices: 'ފުރުއްވި މަޤާމު',
    historical_context: 'ތާރީޚީ ޙާލަތު',
    offices_in_era: 'މި ދައުރުގެ މަޤާމުތައް',
    evidence_strength: 'ހެކީގެ ބާރު',
    evidence_sources_total: 'މަސްދަރު',
    evidence_links_total: 'ގުޅުންތައް',
    crown_office: 'ތާޖު',
    ...
    person_era_label: 'މި ބޭފުޅާގެ ދައުރު',
    no_held_offices: 'މި މީހާގެ މަޤާމެއްގެ ރެކޯޑެއް ނެތް.',
    geographic_context: 'ޖިއޮގްރަފީގެ ޙާލަތު',
    // --- މަޤާމު ފުރަތަމަ ދަރަޖައިގެ ---
    office_summary: 'ޚުލާޞާ',
    office_function_evolution: 'ޒަމާންތަކުގައި ދައުރު',
    office_alt_names: 'އެހެނިހެން ނަން',
    office_holders: 'މަޤާމު ފުރުއްވި ބޭފުޅުން',
    holders_word: 'ފުރުއްވި ބޭފުޅުން',
    no_holders: 'މަޤާމު ފުރުއްވި ބޭފުޅެއް ރެކޯޑެއް ނެތް.',
    office_word: 'މަޤާމު',
    office_detail: 'މަޤާމުގެ ތަފުޞީލު',
    periods_documented: 'ލިޔެކިޔުން ހުރި ދައުރު',
...
const NAME_DV: Map<string, string> = new Map([
...
function normalizeLang(v: string): Locale {
  return v === 'dv' ? 'dv' : 'en';
}
...
export function setLangDirect(lang: Locale): void {
  activeLang = normalizeLang(lang);
}

export function t(key: string): string {
  return D[activeLang]?.[key] ?? D.en[key] ?? key;
}
...
export function personName(personOrId: PersonNode | string, byId?: Map<string, PersonNode>): string {
  const p = typeof personOrId === 'string' ? byId?.get(personOrId) : personOrId;
  if (!p) return typeof personOrId === 'string' ? personOrId : '';
  if (activeLang === 'dv') return NAME_DV.get(p.id) ?? (p as PersonNode & { name_dv?: string }).name_dv ?? p.nm;
  return p.nm;
}
...
function setOptionLabel(sel: string, value: string, label: string): void {
...
function setChipLabel(sel: string, label: string): void {
...
export function refreshChromeLabels(): void {
  setText('vmg', t('graph'));
  setText('vmt', t('tree'));
  ...
  if (input && input instanceof HTMLInputElement) input.placeholder = t('search_placeholder');
...
export function setLanguage(next: string, opts: { silent?: boolean } = {}): void {
  activeLang = normalizeLang(next);
  document.documentElement.lang = activeLang;
  document.body.classList.toggle('lang-dv', activeLang === 'dv');
  const sel = document.getElementById('lng');
  if (sel && sel instanceof HTMLSelectElement) sel.value = activeLang;
  refreshChromeLabels();
  if (!opts.silent) {
    window.dispatchEvent(new CustomEvent('lang-changed', { detail: { lang: activeLang } }));
  }
}

export function initLanguageControl(initial: string = 'en'): void {
  const sel = document.getElementById('lng');
