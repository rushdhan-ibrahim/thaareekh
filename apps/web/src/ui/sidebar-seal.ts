...
let sealContainer: HTMLElement | null = null;
let sealSvg: SVGSVGElement | null = null;
...
export function initSidebarSeal(): void {
  ...
  sealSvg = document.createElementNS(NS, 'svg') as SVGSVGElement;
  ...
  // Insert before the scrollable content area
...
function buildSeal(colorVar: string): void {
  if (!sealSvg) return;
  sealSvg.innerHTML = '';
  ...
  (outer as SVGElement).style.cssText = `animation:spin 90s linear infinite;transform-origin:50px 50px`;
  ...
  // Cardinal ticks
  const cardinals: [number, number][] = [[50, 3], [97, 50], [50, 97], [3, 50]];
  ...
  // 8-fold radials
  const radials: [number, number, number, number][] = [[50, 6, 50, 94], [6, 50, 94, 50], [18, 18, 82, 82], [82, 18, 18, 82]];
  ...
  (inner as SVGElement).style.cssText = `animation:sealBreathe 10s ease-in-out infinite;transform-origin:50px 50px`;
  ...
  (pulseRing as SVGElement).style.animation = 'pulse 6s ease-in-out infinite';

  // Layer 3: Counter-rotating orbit dots (55s)
  const orb = el('g', {}, sealSvg);
  (orb as SVGElement).style.cssText = `animation:spinReverse 55s linear infinite;transform-origin:50px 50px`;
...
export function updateSealColor(dynastyKey: string): void {
  if (!sealSvg) return;
