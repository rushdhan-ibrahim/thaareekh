...
fn payload() -> &'static Payload {
    ...
    PAYLOAD.get_or_init(|| {
        serde_json::from_str(include_str!("../data/phase1_batch_d.payload.json"))
            .expect("valid phase1 batch D payload")
    })
}
...
pub fn run(
    ...
    update_person_coverage(
        ...
        &person_ids,
        date,
        "Phase 1 batch D dossier draft created.",
    )?;
    ...
    update_inference_tracker(
        ...
        &inference_keys,
        date,
        "Phase 1 batch D derived dossier drafted.",
    )?;
    ...
    Ok("Phase 1 batch D complete: dossiers and ledgers updated.\n".to_string())
}
