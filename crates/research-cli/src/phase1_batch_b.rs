use crate::phase1_batch_common::{
    CuratedInferenceNote, DEFAULT_DATE, PersonDossierOptions, QueueUpdate, RelationshipUpdate,
    by_person_id, header_index, inference_edge_key, ledger_dir, make_curated_inference_dossier,
...
fn payload() -> &'static Payload {
    ...
    PAYLOAD.get_or_init(|| {
        serde_json::from_str(include_str!("../data/phase1_batch_b.payload.json"))
            .expect("valid phase1 batch B payload")
    })
}
...
fn write_person_dossiers(
    ...
    let options = PersonDossierOptions {
        ...
        required_edge_updates: "corroborate direct and contested edges with chronicle or inscription-backed wording.",
    };
...
pub fn run(
    ...
    update_person_coverage(
        ...
        &person_ids,
        date,
        "Phase 1 batch B dossier draft created.",
    )?;
    ...
    update_inference_tracker(
        ...
        &inference_keys,
        date,
        "Phase 1 batch B dossier drafted.",
    )?;
    ...
    Ok("Phase 1 batch B complete: dossiers and ledgers updated.\n".to_string())
}
