...
const OWNER: &str = "phase1-locator-b";
...
struct Payload {
    ...
    extract_entries: Vec<HashMap<String, String>>,
...
fn payload() -> &'static Payload {
    ...
    PAYLOAD.get_or_init(|| {
        serde_json::from_str(include_str!("../data/phase1_locator_batch_b.payload.json"))
            .expect("valid phase1 locator batch B payload")
    })
}
...
fn update_relationship_ledger(dataset: &Dataset, path: &Path, date: &str) -> Result<usize, String> {
    let rel_raw =
...
fn upsert_source_extract_log(path: &Path, date: &str) -> Result<usize, String> {
    ...
    for entry in &payload().extract_entries {
    ...
    Ok(payload().extract_entries.len())
}
...
pub fn run(dataset: &Dataset, root_dir: &str, date_opt: Option<&str>) -> Result<String, String> {
    ...
    let claim_count = update_relationship_ledger(dataset, &relationship_path, date)?;
