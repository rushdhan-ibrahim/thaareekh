declare const MovementLibrary: any;
namespace RelicEnhancer {
  type Kind = string;

  const cornerKinds: Kind[] = ['sigilBloom', 'vesica', 'roseWindow', 'pilgrimStar'];
  const miniKinds: Kind[] = ['sigilBloom', 'chaplet', 'vesica', 'labyrinth', 'memoryField', 'roseWindow', 'waterClock', 'pilgrimStar'];
  ...
  function makeLink(href: string, label: string): HTMLAnchorElement {
    ...
    a.href = href;
    a.textContent = label;
    if (currentFile() === href) a.classList.add('is-current');
    return a;
  }

  function injectNavigation(folio: HTMLElement): void {
    if (folio.querySelector('.relic-nav')) return;
    const nav = document.createElement('nav');
    nav.className = 'relic-nav';
    nav.append(
      makeLink('index.html', 'Cabinet'),
      document.createTextNode('·'),
      makeLink('on-the-unfolding-of-form-plates.html', 'Treatise'),
      document.createTextNode('·'),
      makeLink('twelve-movements-for-ink-and-light.html', 'Volume I'),
      document.createTextNode('·'),
      makeLink('twelve-more-movements.html', 'Volume II'),
      document.createTextNode('·'),
      makeLink('twelve-further-artefacts-for-ink-and-light.html', 'Artefacts'),
    );
    folio.insertBefore(nav, folio.firstChild);
  }

  function injectTitleWhisper(folio: HTMLElement): void {
    const titleBlock = folio.querySelector('.title-block');
    if (!titleBlock || titleBlock.querySelector('.relic-whisper')) return;
  ...
  function injectFolioCorners(folio: HTMLElement): void {
    if (folio.querySelector('.relic-folio-corner')) return;
  ...
  function injectPlateFrames(folio: HTMLElement): void {
    const plates = Array.from(folio.querySelectorAll<HTMLElement>('.plate, .geometric-plate'));
    plates.forEach((plate, index) => {
      ...
      let cursor = plate.nextElementSibling as HTMLElement | null;
      while (cursor) {
        ...
        if (cursor.classList.contains('plate') || cursor.classList.contains('geometric-plate')) break;
        cursor = cursor.nextElementSibling as HTMLElement | null;
      }
      if (title && !title.nextElementSibling?.classList.contains('relic-title-tail')) {
  ...
  function injectMarginSigils(folio: HTMLElement): void {
    const notes = Array.from(folio.querySelectorAll<HTMLElement>('.margin-note'));
    notes.forEach((note, index) => {
      if (note.querySelector('.relic-note-sigil')) return;
  ...
  function observeActiveZones(folio: HTMLElement): void {
    const plates = Array.from(folio.querySelectorAll<HTMLElement>('.plate, .geometric-plate, .artefact-plate'));
  ...
  function observeReveals(folio: HTMLElement): void {
    const reveals = Array.from(folio.querySelectorAll<HTMLElement>('.rv, .reveal'));
  ...
  function appendColophonSeal(folio: HTMLElement): void {
    const colophon = folio.querySelector('.colophon');
    if (!colophon || colophon.querySelector('.relic-colophon-seal')) return;
...
(window as any).RelicEnhancer = RelicEnhancer;
