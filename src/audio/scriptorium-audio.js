 ...
 * Mirror of apps/web/src/audio/scriptorium-audio.ts.
 */
import { dynastyPitch, bellPitch, tickPitch, clamp01 } from './audio-maps.js';
...
function lfo(ac, hz, depth, target) {
...
function envGain(ac, { attack = 0.004, decay, peak }) {
  const g = ac.createGain();
...
function tone(freq, type, env, glideTo) {
  if (!ctx) return;
...
function noiseHit(filterType, freq, q, env, sweepTo) {
  if (!ctx) return;
...
export function audioEvent(name, opts = {}) {
