 ...
 * Mirror of apps/web/src/graph/orrery.ts.
 */
...
function svgChild(tag, attrs, parent) {
 ...
 */
export function buildOrrery(ga, cx, cy) {
  if (!ga) return;
...
export function updateOrreryTransform(t) {
  if (!spaceEl || !t) return;
  spaceEl.style.transform = `translate(${t.x}px, ${t.y}px) scale(${t.k})`;
}

/** Pulse a dynasty's era arc when the era filter changes. */
export function pulseOrreryDynasty(_parent, dynastyKey) {
  const arc = hostEl?.querySelector(`.o-ring-${dynastyKey} .o-arc`);
