...
function estimateNodeWidth(node, dens) {
...
export function computeTreePlacement(input) {
  ...
  if (placedTrees.length) {
    ...
    const earlySovColWidth = earlySovereignIsolated.length
      ? (d3.max(earlySovereignIsolated, n => estimateNodeWidth(n, dens)) ?? 0) + 40
      : 0;
  ...
  const hasCollision = (cx, cy, halfW, excludeId) => {
    const margin = 8;
