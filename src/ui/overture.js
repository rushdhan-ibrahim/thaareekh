 ...
 * Mirror of apps/web/src/ui/overture.ts.
 */
...
function nodeYearOf(g) {
  const d = g.__data__;
  if (!d) return 1500;
  const re = d.re;
  if (Array.isArray(re) && re.length) {
    const first = Array.isArray(re[0]) ? re[0][0] : re[0];
...
function inkInNodes() {
  ...
  nodes.forEach(g => {
    ...
    g.style.setProperty('--ink-d', `${Math.round(delay)}ms`);
  });
