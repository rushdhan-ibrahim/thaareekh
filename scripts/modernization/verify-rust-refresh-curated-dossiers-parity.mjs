...
async function prepareWorkspace(dest) {
  ...
  await cp(
    join(projectRoot, 'scripts', 'refresh-curated-inference-dossiers.mjs'),
    join(dest, 'scripts', 'refresh-curated-inference-dossiers.mjs')
  );
...
async function main() {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), 'maldives-curated-dossiers-parity-'));
  ...
  try {
    ...
    run('node', ['scripts/refresh-curated-inference-dossiers.mjs', date], nodeWorkspace);
    run(
      'cargo',
      [
        'run',
    ...
    const dossierFiles = parsed
      .slice(1)
      .filter(row => row[idx.inference_class] === 'curated' && (row[idx.dossier_file] || '').trim())
    ...
    if (issues.length) {
      keepTemp = true;
      console.error(`Refresh curated dossiers parity failed (${issues.length})`);
    ...
    console.log('Refresh curated dossiers parity passed (Node == Rust outputs).');
  } finally {
    if (!keepTemp) {
